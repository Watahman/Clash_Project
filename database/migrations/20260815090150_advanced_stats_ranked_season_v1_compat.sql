-- Rebind the legacy compact write RPC to season-aware composite keys.
-- Existing callers remain valid and default to the active ranked season.
...
begin
    ...
    if v_scope = 'RANKED' then
        ...
        v_season_key := coalesce(v_provenance_season, v_state_season, '');
        v_season_key := public.normalize_advanced_stats_ranked_season_key_v1(v_scope, v_season_key);
            ...
            update public.advanced_stats_scope_state
               set source_season_key = v_season_key,
                   source_provenance = source_provenance || jsonb_build_object(
                       'rankedSeasonKey', v_season_key), updated_at = now()
    ...
    if p_army_hash is not null then
        insert into public.advanced_stats_scope_army_daily (
            tracking_id, scope, season_key, stat_date, army_hash, normalized_army_json,
            battle_count, total_stars, total_destruction
        ) values (
            p_tracking_id, v_scope, v_season_key, v_event_date, p_army_hash, p_normalized_army_json,
    ...
    return jsonb_build_object('inserted', true, 'duplicate', false, 'scope', v_scope,
        'seasonKey', v_season_key, 'eventFingerprint', p_event_fingerprint, 'eventDate', v_event_date);
end;
$$;
