param(
    ...
    [string]$SchedulerJobName = "clashpanel-advanced-stats-poll-preview",
...
if ($SecretName -ne "ADVANCED_STATS_SCHEDULER_SECRET") {
    throw "preview must reuse ADVANCED_STATS_SCHEDULER_SECRET; separate preview secrets are not supported."
}
...
if ($allowlistIds.Count -ne 1) {
    throw "The first preview stage must contain exactly one developer UUID in ADVANCED_STATS_ROLLOUT_USER_IDS."
}
try {
    [void][Guid]::Parse($allowlistIds[0])
} catch {
    throw "The preview rollout allowlist does not contain a valid developer UUID."
}

$jobState = (& gcloud scheduler jobs describe $SchedulerJobName --project $ProjectId --location $Region --format="value(state)").Trim()
if ($LASTEXITCODE -ne 0) {
    throw "Scheduler job '$SchedulerJobName' was not found. Run scripts/deploy/configure-advanced-stats-preview.ps1 first."
}
...
Write-Host "Enabling collection only on the zero-traffic preview candidate..." -ForegroundColor Cyan
...
try {
    ...
    $wrongSecret = Get-HttpResult -Url "$candidateUrl/InternalAdvancedStatsPoll" -Headers @{ "X-ClashPanel-Scheduler-Secret" = "intentionally-wrong-preview-secret" }
    ...
    Write-Warning "preview enablement failed. Applying kill switch on the tagged candidate."
...
Write-Host "Preview developer collection is now enabled on the tagged candidate." -ForegroundColor Green
