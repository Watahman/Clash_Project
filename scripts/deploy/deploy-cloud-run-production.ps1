    ...
    foreach ($name in @('SUPABASE_SERVICE_ROLE_KEY', 'API_PROXY_SECRET', 'ADVANCED_STATS_SCHEDULER_SECRET')) {
        ...
        if ($LASTEXITCODE -ne 0) {
            if ($name -eq 'ADVANCED_STATS_SCHEDULER_SECRET') {
                throw "Secret Manager-secret '$name' ontbreekt. Voer scripts/deploy/configure-advanced-stats-production.ps1 eerst uit; deploy is geen secretrotatie."
            }
...
Write-Host "Deploy klaar. Advanced Stats collection en scheduler blijven uit; start scripts/deploy/configure-advanced-stats-production.ps1 niet zonder aparte releasebeslissing." -ForegroundColor Green
