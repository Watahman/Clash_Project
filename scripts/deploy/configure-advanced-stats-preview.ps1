...
function Get-TaggedCandidateUrl {
    ...
    if (-not $url) {
        throw "Tagged Cloud Run candidate '$TagName' was not found. Run scripts/deploy/deploy-cloud-run-preview.ps1 first."
    }
    return $url
}
...
# Collection intentionally remains disabled until scripts/deploy/enable-advanced-stats-preview.ps1 is run.
...
Write-Host "Preview developer-only runtime configuration prepared." -ForegroundColor Green
...
Write-Host ""
Write-Host "Next: run scripts/deploy/enable-advanced-stats-preview.ps1 to enable collection only on the tagged preview revision." -ForegroundColor Yellow
