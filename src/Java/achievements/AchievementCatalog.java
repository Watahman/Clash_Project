...
public final class AchievementCatalog {
    ...
    private static List<AchievementDefinition> build() {
        ...
        family(items, "village_builder", "Village Builder", "Buildings placed in the Home Village", "base", "home_building_count", 50, 100, 150, 200);
        family(items, "wall_grinder", "Wall Grinder", "Combined Home Village wall levels", "base", "home_wall_level_sum", 500, 1500, 3000, 5000);
        ...
        family(items, "gear_engineer", "Gear Engineer", "Gear-ups detected on the base", "base", "gear_up_count", 1, 2, 3, 4);
        ...
        family(items, "hero_power", "Hero Power", "Combined Home Village hero levels", "army", "home_hero_level_sum", 100, 250, 400, 550);
        ...
        family(items, "army_scholar", "Army Scholar", "Combined Home Village troop levels", "army", "home_unit_level_sum", 75, 150, 250, 350);
        family(items, "spell_scholar", "Spell Scholar", "Combined spell levels", "army", "spell_level_sum", 30, 60, 90, 120);
        family(items, "siege_engineer", "Siege Engineer", "Combined siege machine levels", "army", "siege_level_sum", 10, 20, 30, 40);
        family(items, "pet_keeper", "Pet Keeper", "Pets unlocked", "army", "pet_distinct_count", 2, 5, 8, 10);
        family(items, "pet_trainer", "Pet Trainer", "Combined pet levels", "army", "pet_level_sum", 20, 50, 80, 110);
        ...
        family(items, "equipment_mastery", "Equipment Mastery", "Combined hero equipment levels", "equipment", "equipment_level_sum", 100, 300, 500, 700);
        family(items, "helper_crew", "Helper Crew", "Village helpers unlocked", "progress", "helper_distinct_count", 1, 2, 3, 4);
        family(items, "helper_mastery", "Helper Mastery", "Combined helper levels", "progress", "helper_level_sum", 2, 4, 6, 8);
        ...
        family(items, "builder_wall_grinder", "Builder Wall Grinder", "Combined Builder Base wall levels", "builder_base", "builder_wall_level_sum", 300, 700, 1100, 1500);
        ...
        family(items, "builder_hero_power", "Builder Hero Power", "Combined Builder Base hero levels", "builder_base", "builder_hero_level_sum", 10, 25, 40, 60);
        family(items, "builder_army_power", "Builder Army Power", "Combined Builder Base troop levels", "builder_base", "builder_unit_level_sum", 75, 125, 175, 225);

        family(items, "module_engineer", "Module Engineer", "Defense modules installed", "base", "defense_module_count", 3, 6, 9, 12);
