...
function readPlayerCard(player) { return { name: player.querySelector('.cwl-player-name')?.textContent || '', clanName: player.querySelector('.cwl-player-clan')?.textContent || '', tag: getCardTag(player), townHall: Number(player.dataset.townHall || 1) }; }
function serializePlan() {
    const allClansData=[]; const noClan=[]; const plannerPlayers=[];
    availablePlayers.querySelectorAll('.cwl-player-article[data-planner-card="true"]').forEach(player=>{ const tag=getCardTag(player); if(!tag)return; noClan.push(tag); plannerPlayers.push(readPlayerCard(player)); });
    const pollMeta = getActiveCwlPollMeta();
    allClansData.push({ clanTag:'none', clantag:'none', players:noClan, groupId: pollMeta.groupId || '', pollId: pollMeta.pollId || '' });
    allClans.querySelectorAll('.cwl-clan-article').forEach(clan=>{ const clanName=clan.querySelector('.cwl-clan-name')?.textContent||''; const clanTag=localStorage.getItem('clanId_'+clanName)||clan.dataset.clanTag||''; const amountOfPlayers=clan.querySelector('.cwl-amount-of-players-in-clan')?.textContent?.split('/')[1]||'15'; const allPlayersInClan=[]; clan.querySelectorAll('.cwl-player-article[data-planner-card="true"]').forEach(player=>{ const tag=getCardTag(player); if(!tag)return; allPlayersInClan.push(tag); plannerPlayers.push(readPlayerCard(player)); }); allClansData.push({ clantag:clanTag, clanTag, amountOfPlayers, uuid: clan.id.split('_').at(-1), players: allPlayersInClan }); });
...
function loadPlanById(planId){ const token=++activeLoadToken; suppressSave=true; setCanAutosave(false); setLoading(true); setSaveStatus('idle'); const cached=planCache.get(planId); const loadPromise=cached?.info?Promise.resolve(cached):(fullPlanPromises.get(planId)||getPlanFromDatabase(planId).then(normalizePlan)); withGlobalLoading(() => loadPromise.then(data=>{ if(token!==activeLoadToken)return; const normalized=normalizePlan(data); if(!normalized) throw new Error('Ongeldig plan'); planCache.set(normalized.id,normalized); return renderLoadedPlan(normalized,normalized.id,token); }).catch(error=>{ if(token!==activeLoadToken)return; console.error(error); setSaveStatus('error'); }), t('cwl.loadingPlans')).finally(()=>{ if(token!==activeLoadToken)return; setLoading(false); setCanAutosave(true); suppressSave=false; window.dispatchEvent(new CustomEvent('clashtools:cwl-plan-loaded')); }); }
function renderLoadedPlan(data,planId,token){ availablePlayers.innerHTML=''; allClans.innerHTML=''; totalPlayerAmount.textContent='0'; planName.value=data.name||''; localStorage.setItem('planner_id',data.id||planId); const info=Array.isArray(data.info)?data.info:[]; const freePlayers=info[0]?.players||[]; const clansToLoad=info.slice(1); const tasks=[]; freePlayers.forEach(playerTag=>tasks.push(getPlayerBasicData(playerTag).then(playerData=>{ if(token===activeLoadToken) createPlayerCard(playerData,null); }).catch(console.error))); clansToLoad.forEach(clan=>{ const clanTag=clan.clantag||clan.clanTag; if(!clanTag)return; tasks.push(getClanInfoRequest(clanTag).then(clanData=>{ if(token!==activeLoadToken)return; createClanCard(clanData,clan.amountOfPlayers,clan.uuid); return Promise.allSettled((clan.players||[]).map(playerTag=>getPlayerBasicData(playerTag).then(playerData=>{ if(token===activeLoadToken) createPlayerCard(playerData,clan.uuid); }).catch(console.error))); }).catch(console.error)); }); return Promise.allSettled(tasks).then(()=>{ if(token===activeLoadToken){ window.dispatchEvent(new CustomEvent('clashtools:cwl-plan-meta-loaded',{ detail:{ groupId:info[0]?.groupId||'', pollId:info[0]?.pollId||'' }})); serializePlan(); } }); }
