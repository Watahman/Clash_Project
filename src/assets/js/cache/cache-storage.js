...
async function migrateLegacyCache(database) {
    try {
        if (globalThis.localStorage?.getItem(MIGRATION_MARKER) === '1') return;
        const entries = Object.keys(globalThis.localStorage || {})
            .filter(key => key.startsWith(LEGACY_STORAGE_PREFIX))
            .map(key => {
                try {
                    return [key, JSON.parse(globalThis.localStorage.getItem(key))];
                } catch {
                    return [key, null];
                }
            })
            .filter(([, entry]) => isValidCacheEntry(entry));
        ...
        entries.forEach(([key]) => globalThis.localStorage.removeItem(key));
        globalThis.localStorage.setItem(MIGRATION_MARKER, '1');
    } catch {
...
export async function writeCacheEntry(entry, retry = true) {
    const database = await openDatabase();
    if (!database) {
    ...
    try {
        ...
        if (retry && (error?.name === 'QuotaExceededError' || error?.name === 'UnknownError')) {
            ...
            return writeCacheEntry(entry, false);
        }
        memoryFallback.set(entry.key, entry);
        return entry;
    }
...
export async function clearCacheEntries(predicate) {
    for (const [key, entry] of memoryFallback) {
        if (predicate(entry, key)) memoryFallback.delete(key);
    }
    const database = await openDatabase();
    if (!database) return;
    try {
        ...
        cursorRequest.onsuccess = () => {
            ...
            if (!cursor) return;
            if (predicate(cursor.value, cursor.key)) cursor.delete();
            cursor.continue();
        };
        await transactionComplete(transaction);
    } catch {
...
async function cleanupCache(database, force = false) {
    ...
    try {
        ...
        const keysToDelete = entries
            .filter(entry => force || !isValidCacheEntry(entry) || entry.expiresAt <= currentTime)
            .map(entry => entry.key);
        const retained = entries
            .filter(entry => isValidCacheEntry(entry) && entry.expiresAt > currentTime)
