...
function bindMapControls(root, onMapSide, renderCurrent) {
    ...
    const switcher = root.querySelector('.war-side-switch');
    if (!liveMap || !switcher) return;
    switcher.addEventListener('click', event => {
        const button = event.target.closest('[data-map-side]');
        if (!button || button.dataset.mapSide === liveMap.dataset.warMapSide) return;
        animateMapSideSwitch(liveMap, button.dataset.mapSide);
        onMapSide(button.dataset.mapSide);
        switcher.querySelectorAll('[data-map-side]').forEach(item =>
