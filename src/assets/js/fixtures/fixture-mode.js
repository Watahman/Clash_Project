...
const CATALOG_URL = '/fixtures/development/scenarios.json';
...
export async function getFixture(location = window.location) {
    const id = requestedFixtureId(location);
    if (!id) return null;
    if (!isLocalFixtureHost(location)) {
        throw new Error('Fixtures are restricted to localhost.');
    }
...
export async function initFixtureMode() {
    if (!requestedFixtureId()) return null;
    try {
        const fixture = await getFixture();
        document.documentElement.dataset.fixture = fixture.id;
...
if (typeof window !== 'undefined' && typeof document !== 'undefined') {
    const start = () => void initFixtureMode().catch(error => {
