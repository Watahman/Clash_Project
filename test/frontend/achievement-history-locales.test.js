...
describe('Achievement history and locale integration', () => {
    ...
    it('loads a permanent achievements link through the central registry', () => {
        const navigation = readFileSync('src/assets/js/shell/module-registry.js', 'utf8');
        const runtimeTranslations = readFileSync('src/assets/js/i18n/runtime-translations.js', 'utf8');
        expect(navigation).toContain("['achievements', 'nav.achievements'");
        expect(navigation).toContain("'/app/achievements'");
        expect(runtimeTranslations).not.toContain('achievements-navigation.js');
    });
