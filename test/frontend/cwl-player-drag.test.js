...
describe('CWL player drag interaction', () => {
    ...
    it('uses a placeholder and restores an invalid drop', () => {
        ...
        expect(card.classList.contains('cwl-player-drag-source')).toBe(true);
        expect(source.querySelector('.cwl-player-drag-preview')).not.toBeNull();

        dispatchPointer(document, 'pointerup', 700, 500);
        expect(card.parentElement).toBe(source);
        expect(source.firstElementChild).toBe(card);
        expect(document.querySelector('.cwl-player-drag-preview')).toBeNull();
        expect(card.releasePointerCapture).toHaveBeenCalledWith(1);
    });
    ...
    it('restores the card when the captured pointer is cancelled', () => {
        ...
        expect(card.parentElement).toBe(source);
        expect(card.classList.contains('cwl-player-drag-source')).toBe(false);
        expect(document.querySelector('.cwl-player-drag-preview')).toBeNull();
    });
});
